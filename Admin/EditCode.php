...
if(isset($_POST['cardServiceBtn'])){
    ...
    if($end_time <= $start_time){
        ...
        $_SESSION['warning'] = "You can not select same or less time to the start Time ($end_time).";
    }else{
        $query = mysqli_query($con, "INSERT INTO `services`(`title`, `date`, `start_time`, `end_time`, `link`) 
        VALUES ('$title','$date','$start_time','$end_time','$link')");
        if($query){
            echo "<script>window.open('services.php','_self')</script>";
...
if(isset($_POST['editSelectedService'])){
    if (isset($_POST['selected']) && !empty($_POST['selected'])) {
        ...
        foreach ($selected_ids as $id) {
            ...
            $query = mysqli_query($con, "UPDATE `services` SET `title` = '$title', 
            `date` = '$date', 
        ...
        echo "<script>window.open('services.php','_self')</script>";
        $_SESSION['warning'] = "No Columns has selected.";
    }
}
